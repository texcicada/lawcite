...
%
\newtoggle{lcindexactualequal}
\toggletrue{lcindexactualequal}
...
% case fields
  ...
  indexyear, %decision year in plain format
...
\DeclareBibliographyOption[boolean]{index-actual-equal}[false]{%
\ifstrequal{#1}{true}
{\toggletrue{lcindexactualequal}}
{\togglefalse{lcindexactualequal}}%
}
...
\newbibmacro{getcasedecisionyearplain}{%
\iffieldundef{indexyear}{}{\printfield[plain]{indexyear}}%
}


%============= lcdecisionyear: plain
\DeclareCiteCommand{lcdecisionyear}%
%@@@@@@@@@@@@@@@@ prenote
{\usebibmacro{prenote}}%
%@@@@@@@@@@@@@@@@ item
{%
...
%\usebibmacro{fullstepindex}%
\usebibmacro{sendtocaseindex}%
\usebibmacro{getcasedecisionyearplain}%
}%end item
%@@@@@@@@@@@@@@@@ delim
{\multicitedelim}%
%@@@@@@@@@@@@@@@@ postnote
...
\noindent\ifnameundef{author}{}{author =  \{\printnames[authorplainlast][1-1]{author}\},\%1st only\par}%
...
\noindent\iffieldundef{crossref}{}{crossref =  \{\thefield{crossref}\},\par}%
\noindent\iffieldundef{date}{}{day =  \{\thefield{date}\},\par}%\noindent\iffieldundef{day}{}{day =  \{\thefield{day}\},\par}%
...
\noindent\iflistundef{location}{}{location =  \{\thelist{location}\},\par}%
...
\noindent\iflistundef{publisher}{}{publisher =  \{\thelist{publisher}\},\par}%
...
\sindex[cases]{%
\thefield{partya}\thefield{partyb}\iftoggle{lcindexactualequal}{=}{@}%for sorting
%partynames = casename
...
\usepackage[
  ...
  index-actual-equal=true,%index style file uses: actual '=' instead of @ for sortkey/actual value
	indexing=false,%cite,
	citetracker=true,
	citecounter=context,
...
\usepackage{splitidx}
\newcommand\pagerefindexnote{\bigskip\noindent\small\mdseries $\to$ References are to \iftoggle{lcparnumrefs}{paragraph}{page} numbers.\bigskip}
...
\AtWriteToIndex{idx}{\let\thepage\theparno}
%-------------------------------------------------------
%Note that this will work only if the shortcut of the index is given explicitly in each marked-up index entry...
...
%-------------------------------
\newcommand\examplefieldscite[4][\lcdcase]{%
{\small
\begin{tabular}{ll}
...
\hline & \\
\fcolorbox{violet}{brown!18}{\fcolorbox{violet}{brown!12}{\begin{minipage}{0.4\textwidth}
\bemph{#2} $\mapsto$\ \par
\smallskip
#3
\end{minipage}}} 
...
%https://tex.stackexchange.com/questions/603635/trying-to-get-different-backgrounds-on-even-and-odd-pages:
%    \AddEverypageHook{⟨code⟩} ≡ \AddToHook{shipout/background}{\put(1in,-1in){⟨code⟩}}


%%%\newcounter{rectmark}
%%%
%%%\newcommand{\printrectanglea}{\stepcounter{rectmark}
%%%  \gdef\mynode{pic cs:\therectmark}\tikzmark{\therectmark}\textcolor{red}{TL}}%
%%%\newcommand{\printrectangleb}{%\stepcounter{rectmark}
%%%  \gdef\mynodebr{pic cs:br\therectmark}\tikzmark{br\therectmark}\textcolor{red}{BR}}%
%%%  
%%%\newcommand{\printrectanglec}{%\stepcounter{rectmark}
...
%%%%    ( $ (\mynode) + (\paperwidth,0) $ );
%%%    \fill[blue!10!white] (\mynode)[xshift=-2em] rectangle 
%%%    									(\mynodebr)[xshift=\textwidth];
%%%  \end{tikzpicture}
%%%  }}
%%%\gdef\mynode{0,0}%
%%%\gdef\mynodebr{0,0}%
...
\p This document uses paragraph numbers.\footnote{The SplitIndex command \texttt{AtWriteToIndex} has been set to the paragraph number for all indices: \cmd{AtWriteToIndex}\braces{idx}\braces{\cmd{let}\cmd{thepage}\cmd{theparno}} etc; the square.ist index style file is being used instead of plain.ist; and the \texttt{use-toc-parnumrefs} biblatex option has been set to true.}
...
\newpage
\p Citing a law journal or law review.

\examplefieldscite{\cmd{ljfootcite}\brackets{276}\braces{kleefeld}}{``typography matters''\ljfootcite[276]{kleefeld}}{kleefeld}
...
\newpage
\p Citing a legal monograph.

\examplefieldscite[\lcdallfields]{{\scriptsize\cmd{lcbook}\brackets{12}\hspace{0.1em}\braces{addison}}}{``Questions of proprietary right often involve nice distinctions.''\lcbook[12]{addison}}{addison}


\newpage
\p A quotation from a case.

{
\lcsetstylemcgill
\exampledemo[fsoff]{\lccurrentstyle}{\mkbibquote{\ldots Chief Justice Lamer’s observations in \lcshorttitle{rta1996} where he invoked Lord Sankey’s famous admonition in the \lcdecisionyear{edwardsa} \lcnickname{edwardsa} case \lcinlineparens{edwardsa} that the Constitution is a \mkbibquote{living tree} to be interpreted flexibly\ldots}\lawcite[From][303]{ccpref}}{Canadian style}{can}
}
...
\p A non-legal cite\autocite{westfahl:space}\sindex[idx]{Westfahl}. 

\p To test the ibid mechanism, change the ibidtracker option for biblatex to true:\sindex[general]{ibid test}
\begin{verbatim}ibidtracker=true,\end{verbatim} and then re-run.

\p An ibid test: text case\lawcite{lchousen2002} text ibid\lawcite[3]{lchousen2002} text next\lawcite[100]{lccrestonmoly2014}
...
%======================================= Bibliography
\newpage
\p The McGill bibliography.\paradef{mcgbib}
...
\newpage
\p Non-McGill material: a Table of Cases (run with SplitIndex, using the squareh.ist index style file). Note that ToC is for other legal citation styles, and that items appearing in a McGill bibliography (see \pararef{mcgbib}) count as citations for the index tracker.
...
https://tex.stackexchange.com/questions/233831/modifying-the-oscola-citation-style/603686#603686