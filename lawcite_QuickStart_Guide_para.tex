...
\p Using the ext-authortitle-ibid Biblatex style as a base, lawcite defines three new bibentry types, @case, @statute and @ljarticle, and a (more or less) flexible citation command (\cmd{lawcite} itself) plus some ancillary commands. 
...
\p The lc-proceedings citation command, \cmd{lcproc}, looks for the bibentry fields shown in code listing \ref{ex}.
...
\p \cmdb{lcproc}{abcproc} produces: 
...
%\newpage
\p The \cmd{lawcite} command's indexing capability has been enhanced to also include page hyperlinking in the Table of Cases, like \cmd{lcproc} does.
...
\p The \cmd{lawcite} command is the general purpose citation command.
