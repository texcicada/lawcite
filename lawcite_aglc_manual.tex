...
@case{finn,partya = {Director of Public Prosecutions},partyb = {Finn},partyasn = {DPP},partyajur = {Vic},reportyear = {2008},reportvolume = {186},reportseries = {A Crim R},reportpage = {235},}
@case{bradshaw,partya = {Bradshaw},partyb = {Attorney-General},partybsn = {A-G},partybjur = {Qld},reportyear = {2000},volyearneeded = {true},reportvolume = {2},reportseries = {Qd R},reportpage = {7},}

@case{thomasmowbray,partya = {Thomas},partyb = {Mowbray},reportyear = {2007},reportvolume = {233},reportseries = {CLR},reportpage = {307},}
@book{aglc,title = {Australian Guide to Legal Citation},edition = {4},date = {2019},publisher = {Melbourne University Law Review Association Inc},location = {Melbourne},isbn = {9780646976389},shorttitle = {AGLC},}

@statute{cca,statutetitle = {Criminal Code Act},statutetitleyear = {1995},statutejurisdiction = {Cth},}
...
\title{AGLC Legal Citation\\Usage Guide\\ \ \\{\normalsize for \\ the \texttt{lawcite} \textsc{Biblatex} style}}
\author{}
\date{}
\long\gdef\myabstracttext{\noindent This is the instruction manual for using the  (\cmdc{lawcite})  \textsc{Biblatex} format to produce AGLC-style legal citations.}
%------------------
...
AGLC-style is activated via option settings when calling the Biblatex package.
...
\subsection{AGLC settings}
For a behind-the-scenes look, in terms of on/off switches (that is, where there is a typographical choice), the AGLC style is defined as follows: party names are italic, party separator (usually a ``v'') is also italic, and not dotted, main citation method is footnoting, square brackets are used for the year component of medium neutral citations, and so on\ldots
\begin{verbatim}
...
\section{Cases}
\subsection{Bibentries}
This subsection illustrates the bibentry data fields driving the citations,
\bigskip

\theball Short name in footer: \cmd{lawcitetitle}\braces{bradshaw}\cmd{lawcite}\braces{bradshaw}


In \lawcitetitle{bradshaw}\lawcite{bradshaw} \ldots ({\small AGLC example})


\begin{verbatim}
@case{bradshaw,
	partya = {Bradshaw},
	partyb = {Attorney-General},
	partybsn = {A-G},
	partybjur = {Qld},
	reportyear = {2000},
	volyearneeded = {true},
...
\cmd{lawcite} is a footer style under AGLC. Use the \cmd{lawcitehf} command to explicitly place the case name inline and have the reference in a footer with one command, like so:\footcite[The example is from][r 2.1.15 `Omitting the Case Name' p 48]{aglc}
\begin{quotation}
In \lawcitehf{thomasmowbray}, the control order regime established under the schedule to the \lawcite{cca} survived a constitutional challenge. 
\end{quotation}
\bigskip

Use \cmd{lawcitehfn} for the short name in the footer.

In \lawcitehfn{finn} \ldots
\bigskip


\theball Medium Neutral Citation: \cmd{lcinline}\braces{cole}\par\bigskip
 \lcinline{cole}
\begin{verbatim}
@case{cole,
	partya = {R},
...
\theball The default party separator is ``v''. Use the \textit{partysep=} field to specify a different party separator, e.g., \textit{contre}: \cmd{lcinline}\braces{arbia}\par\bigskip
 \lcinline{arbia}
...
\theball Printed report, by year: \cmd{lcinline}\braces{hopp}\par\bigskip
 \lcinline{hopp}
\begin{verbatim}
@case{hopp,
	partya = {Hopp},
...
\theball Printed report, by volume: \cmd{lcinline}\braces{fucella}\par\bigskip
 \lcinline{fucella}
\begin{verbatim}
@case{fucella,
	partya = {Fucella},
...
\theball Printed report, by year, decided and published in different years: \cmd{lcinline}\braces{swissair}\par\bigskip
\lcinline{swissair}
\begin{verbatim}
@case{swissair,
...
\end{verbatim}
\newpage
\theball Parallel reports: \cmd{lawcitesinlinerr}\braces{grahamwwr,grahamdlr}\par\bigskip
...
%	keyword=lawbook,
	...
	nottype=statute,
	category=yesinbib,
	title={\center Articles/Books/Reports},
...
\printbibliography[category=notinbib,
title={Quotation sources}]
