...
Splitindex is used for the Table of Cases and Table of Statutes (in actualiy, indices). To apply styling on indices coming from a file \textit{foo}, do: 
...
Note that database entries that look like medium neutral citations (like \lcrefnn{armstrongmnc}) are stored in the .bib file, and therefore processed by the citations commands, as if they were MNCs, but they will not have internal paragraph numbers or page numbers.
...
Given that the citation can split into two, the prenote and postnote need to stay with the head and tail, and so are moved into the \textit{loop} parameter, and then a case statement is applied.
...
Citing unrelated multiple keys in the footnote:\marginnote{\refstepcounter{democount}(d.\thedemocount)}
\begin{filecontents}[overwrite,nosearch,noheader]{\democodefile}
...
Citing unrelated multiple keys, with multicites, in the footnote:\marginnote{\refstepcounter{democount}(d.\thedemocount)}
\begin{filecontents}[overwrite,nosearch,noheader]{\democodefile}
...
%%Dynamic entry set: for the bibliography
%%%\defbibentryset{becherset}{columbussr,columbusne}
%%\defbibentryset{becherset}{keyabc,case2}
%%
%%xdata: \lawcitesinlinerr(Zimmerman J, for the court)[\nopp 200]{xcolumbussr}[\nopp 838]{xcolumbusne} 
%%%\lcinline{becherset}
...
%==========================
%\renewcommand\egcasename\egcasename{City of Marion v Becker}
%\renewcommand\egcaseref{(1973) 6 SASR 13, 29 (Bray CJ)}
...
``it has been argued\footnote{By \ljcite[\nopp 785]{friedman}, cited by \lcljauthor{smythsc}.} that, stylistically,
...
%\section{The citeref= Method}
%Statutechapter method (canreg):


%it has been argued that, stylistically,
%dissents are often looser than majority judgments.
%Russell Smyth
%WHAT DO INTERMEDIATE APPELLATE COURTS
%CITE? A QUANTITATIVE STUDY OF THE CITATION
%PRACTICE OF AUSTRALIAN STATE SUPREME COURTS
%  (1999) 21 AdelLR 51-80, 59
%[1999] AdelLawRw 3; (1999) 21 Adelaide Law Review 51
%Lawrence Friedman, Robert Kagan, Bliss Cartwright and Stanton
%Wheeler, 'State Supreme Courts: A Century of Style and Citation' (1981) 33 Stanford Law
%Review 773 (covering 16 state supreme courts in the period 1870-1970).
%785
...
%
%\section{The Elements of a Legal Citation -- Legislation}
%xxx
%
%
%Becker\footnote{Butt on Legal Usage, 295}
%Cassie\footnote{Butt on Legal Usage, 187}
%Becher\footnote{Butt on Legal Usage, 295}
%Morgan\footnote{AnnCrimLegNSW 2014-2015 para [8-s 249B.5], p 1070}
