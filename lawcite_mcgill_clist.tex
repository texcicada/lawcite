...
%``The Commonwealth's submission \ldots is glib and unhelpful[151].''\lawcite[99]{ajl20}

%``the Bivens action for damages[153] is not recognised in Australia[154].''\lawcite[99]{ajl20} 

\newpage
{
\lcsetstyleaglc
\exampledemo[fsoff]{\lccurrentstyle}{``the \lccasesn{bivens} action for damages\lcfootfull[\cemph{After}\tikzmark{aftera}][]{bivens} is not recognised in Australia\footnote{\lcinline[\lcat 53 \mkbibbrackets{40}]{bata}, \lcinline[\cemph{citing}][\lcat 46--47, 93, 125‑-126, 146--148]{kruger}.}.''\lawcite[From][\lcat \mkbibbrackets{99}]{ajl20}}{Australian style}{aus}
}
...
\bigskip
{\ttfamily
``the \cmd{lccasesn}\braces{\bemph{bivens}} action for damages\cmd{lcfootfull} \brackets{\cemph{After}\tikzmark {afterb}} \brackets{} \braces{\bemph{bivens}} is not recognised in Australia\cmd{footnote \braces{\cmd{lcinline} \brackets{\cmd{lcat} 53 \cmd{mkbibbrackets} \braces{40}} \braces{bata}, \cmd{lcinline} \brackets{\cemph{citing}} \brackets{\cmd{lcat} 46--47, 93, 125‑-126, 146--148}\braces{kruger}.}.''\cmd{lawcite} \brackets{From} \brackets{\cmd{lcat} \cmd{mkbibbrackets}\braces{99}} \braces{ajl20}}
}

\bigskip
With the first reference here to \lccasesn{bivens} (using \cmd{lccasesn}), a second reference, in the footnote, would \textit{ibid} out, so a full reference (by way of \cmd{lcfootfull}) is used instead.

\begin{tikzpicture}[remember picture, overlay]
\draw [->] (pic cs:aftera) edge (pic cs:afterb);
\end{tikzpicture}
%153:
% After Bivens v Six Unknown Named Agents of Federal Bureau of Narcotics [1971] USSC 133; (1971) 403 US 388.

%154
%British American Tobacco Australia Ltd v Western Australia [2003] HCA 47; (2003) 217 CLR 30 at 52 [40], citing Kruger v The Commonwealth [1997] HCA 27; (1997) 190 CLR 1 at 46-47, 93, 125‑126, 146-148. 


%Chief Justice Lamer’s observations in Residential Tenancies (1996) where he invoked Lord Sankey’s famous admonition in the 1929 “Persons” case (Edwards v. Attorney-General for Canada, 1929 CanLII 438 (UK JCPC), [1930] A.C. 124 (P.C.)) that the Constitution is a “living tree” to be interpreted flexibly
%
%303
